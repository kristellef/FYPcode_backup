...
for i in data:
    black_nodes.add(i)
    if str(i)[0:5] not in dict_black_nodes.keys():
        dict_black_nodes[str(i)[0:5]]=set()
    dict_black_nodes[str(i)[0:5]].add(i)
...
def print_red(options, blk_id):
    ...
    if not options.pickled:
        blk = block.load_id(None, int(blk_id), "/Volumes/KIKS/UH-0/UH-%s-%s"%(options.currency, options.heuristic))
    else:
    ...
    for (pos, ( transaction_hash, transaction_fee, new_elements, transaction_input,transaction_output) ) in enumerate(blk.transactions):
        ...
        for i in inputs:
            ...
            if str_i[0:4] in dict_black_nodes.keys():
                if i in dict_black_nodes[str_i[0:5]]:
                    count_tx_black+=1
                    number_black_tx_in_block+=1
                    ...
                    for v in transaction_output:
                        ...
                        if str_v[0:5] not in dict_black_nodes.keys():
                            dict_black_nodes[str_v[0:5]]=set()
                        dict_black_nodes[str_v[0:5]].add(i)
...
args=[]
path = '/Volumes/KIKS/UH-0/UH-bitcoin-heur_0/'
for filename in os.listdir(path):
    if path+filename=='/Volumes/KIKS/UH-0/UH-bitcoin-heur_0/.DS_Store':
        continue
...
for a in args:
    ...
    #     np.save('/Users/macbook/desktop/FYP/heur0-V4'+str(args.index(a)), np.array(y))
    print_red(opts, a)

# np.save('/Users/macbook/desktop/FYP/heur0-V4', np.array(y))
