...
count_tx_black=0
count_total_tx=0
data = np.load('/Users/macbook/Desktop/FYP/files/darknet_minimise_0to1 V2.npy')
...
def print_red(options, blk_id):
    ...
    if not options.pickled:
        blk = block.load_id(None, int(blk_id), "/Volumes/KIKS/UH-1/UH-%s-%s"%(options.currency, options.heuristic))
    else:
...
args=[]
path = '/Volumes/KIKS/UH-1/UH-bitcoin-heur_1/'
for filename in os.listdir(path):
    if path+filename=='/Volumes/KIKS/UH-1/UH-bitcoin-heur_1/.DS_Store':
        continue
...
for a in args:
    ...
    if args.index(a)%100000==0:
        np.save('/Users/macbook/desktop/FYP/heur1-V4'+str(args.index(a)), np.array(y))
    print_red(opts, a)

np.save('/Users/macbook/desktop/FYP/heur1-V4', np.array(y))
