...
transactions = []
for row in reader:
    if len(row) == 7 and row[0] != 'date' and row[2]=='' and row[4]!='(fee)':
...
while index < numberTransactions:
    ...
    if transaction[0][:10] != currentDate:
        ...
        for i in range(0,len(currentTransactions)):
            temp.append(float(currentTransactions[i][3]))
...
data = [trace1, trace2]
layout = go.Layout(
    title='Outflow transaction pattern of AlphaBayMarket by day',
    yaxis=dict(
...
plot_url = py.plot(fig, filename='Median-tx-amount-AlphaBayMarket-outflow')
...
layout = dict(title = 'Outflow transaction pattern of AlphaBayMarket by day',
              yaxis = dict(title='Median of Tx Interval (in minutes)'),
              xaxis = dict(title='Median of Received Amount (in BTC)')
             )

fig = dict(data=data, layout=layout)
py.plot(fig, filename='scatter-median-AlphaBayMarket-outflow')