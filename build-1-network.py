...
#        self.file_out = open("NET-%s-%s.%s.info"%(self.currency, self.heuristic, self.epoch), "w")
        ...
        if self.currency != "ethereum":
            ...
            self.naddr_nstd = None
            self.naddr_nspk = None
            self.naddr_ngen = None
    ...
    def process(self, blk):
        # for each block add 1 to each transaction
...
#        self.file_out.flush()
        if not os.path.exists( "%s/%s"%(out_dir, fout_name) ):
...
if __name__ == "__main__":
    # Parse file arguments (network builder options), create a network builder and process
